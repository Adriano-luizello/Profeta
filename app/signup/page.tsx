  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        ...
        alert("Sucesso! Verifique seu email para confirmar sua conta.");
        router.push("/login");
      }
    } catch (err) {
    ...
    <div className="min-h-screen flex flex-col md:flex-row">
      ...
      <div className="md:w-[60%] bg-profeta-green flex flex-col justify-center px-8 py-12 md:py-24 relative overflow-hidden">
        ...
        <div className="relative">
          ...
          <p className="text-white/90 text-lg max-w-sm">{TAGLINE_PT}</p>
          <p className="text-white/70 text-sm mt-6 hidden md:block">{TAGLINE_EN}</p>
        </div>
      </div>
      ...
      <div className="md:w-[40%] flex items-center justify-center bg-white p-6 md:p-12">
        <div className="w-full max-w-sm">
          ...
          <form onSubmit={handleSignup} className="space-y-5">
            ...
            <div>
              <label htmlFor="name" className="block text-sm font-medium text-profeta-primary mb-1.5">
                Nome
              </label>
              <input
                id="name"
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-3 rounded-xl border border-profeta-border bg-white text-profeta-primary placeholder-profeta-muted focus:outline-none focus:ring-2 focus:ring-profeta-green/20 focus:border-profeta-green"
              />
              <p className="mt-1 text-xs text-profeta-muted">Mínimo 6 caracteres</p>
            </div>

            <div>
              <label htmlFor="confirmPassword" className="block text-sm font-medium text-profeta-primary mb-1.5">
                Confirmar senha
              </label>
              <input
                id="confirmPassword"
          ...
          <p className="mt-8 text-center text-sm text-profeta-secondary">
            Já tem conta?{" "}
            <Link href="/login" className="text-profeta-green font-medium hover:underline">
              Entrar
            </Link>
          </p>

          <p className="mt-6 text-center text-xs text-profeta-muted">
